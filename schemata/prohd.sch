...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern id="pipeCharacter">
        <rule context="tei:*">
            <report test="text()[contains(., '|')]" role="WARNING"> [W0001] The uncommon character
    ...
    <pattern id="allTextNodes">
        <rule context="text()[contains(., '@')]">
            <report test="ancestor::tei:text" role="WARNING"> [W0003] The uncommon character '@' has
    ...
    <pattern id="choiceElement">
        <rule context="tei:choice">
            <assert test="count(*) > 1" role="ERROR"> [E0001] Element " <name/> " must have at least
    ...
    <pattern id="choiceSubelements">
        <rule context="tei:corr | tei:abbr | tei:reg | tei:sic | tei:expan">
            <assert test="parent::tei:choice" role="ERROR"> [E0002] Element " <name/> " must have a
    ...
    <pattern id="corrElement">
        <rule context="tei:corr">
            <assert test="count(preceding-sibling::tei:sic | following-sibling::tei:sic) = 1"
                role="ERROR"> [E0003] Element " <name/> " must have exactly one corresponding "sic"
                element. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="correspAttribute">
        <rule context="tei:*[@corresp]">
            <assert test="matches(@corresp, '^#|^https?://')" role="ERROR"> [E0004] The value of
            ...
            <assert test="
                    if (starts-with(@corresp, '#')) then
                        ...
                        1" role="error"> [E0005] The value of attribute @corresp must
    ...
    <pattern id="expanElement">
        <rule context="tei:expan">
            <assert test="count(preceding-sibling::tei:abbr | following-sibling::tei:abbr) = 1"
                role="ERROR"> [E0006] Element " <name/> " must have exactly one corresponding "abbr"
                element. </assert>
        </rule>
    </pattern>
        ...
        <rule context="tei:pb[@facs]">
            <assert test="
                    if (matches(@facs, '^#f\d\d\d\d') and matches(preceding::tei:pb[1]/@facs, '^#f\d\d\d\d') and (preceding::tei:pb)) then
                        ...
                        1" role="ERROR"> [E0007] Value of @facs within "pb" incorrect;
    ...
    <pattern id="facsOutsidePagebreaks">
        <rule context="tei:*[@facs][not(self::tei:pb)]">
            <assert test="matches(@facs, '^#|^https?://')" role="ERROR"> [E0008] The value of
    ...
    <pattern id="hiRendRendition">
        <rule context="tei:hi">
            <report test="(@rendition and @rend) and @rendition != '#none'" role="WARNING"> [W0004]
    ...
    <pattern id="kValueInRenditionAttribute">
        <rule context="tei:*[@rendition = '#k']">
            <report test="contains(self::tei:*, 'ſ')" role="ERROR"> [E0009] Long s not allowed
        ...
        <rule context="tei:*[@next]">
            ...
            <assert test="
                    if (starts-with(@next, '#')) then
                        ...
                        1" role="error"> [E0010] The value of attribute @next must have
...
-->
    ...
    <pattern id="prevAttribute">
        <rule context="tei:*[@prev]">
            <assert test="starts-with(@prev, '#')" role="ERROR"> [E0011] The value of attribute
                @prev must be a same document reference starting with '#'. </assert>
            <assert test="
                    if (starts-with(@prev, '#')) then
                        //@xml:id = substring-after(@prev, '#')
                    else
                        1" role="error"> [E0012] The value of attribute @prev must have
                a corresponding @xml:id-value within the same document. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="regElement">
        <rule context="tei:reg">
            <assert test="count(preceding-sibling::tei:orig | following-sibling::tei:orig) = 1"
                role="ERROR"> [E0013] Element " <name/> " must have exactly one corresponding "orig"
                element. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="salute">
        <rule context="tei:salute">
            <assert test="ancestor::tei:opener | ancestor::tei:closer"> [E0014] The element "salute"
    ...
    <pattern id="sicElement">
        <rule context="tei:sic">
            <assert test="count(preceding-sibling::tei:corr | following-sibling::tei:corr) = 1"
                role="ERROR"> [E0015] Element " <name/> " must have exactly one corresponding "corr"
                element. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="signed">
        <rule context="tei:signed">
            <assert test="ancestor::tei:closer"> [E0016] The element "signed" may only occur within
    ...
    <pattern id="subst">
        <rule context="tei:subst">
            <assert test="child::tei:add and child::tei:del"> [E0017] The element "subst" must
                contain both elements "add" and "del" as child elements. </assert>
        </rule>
    </pattern>

    <pattern id="targetAttribute">
        <rule context="tei:*[@target] except tei:catRef">
            <assert test="matches(@target, '^#|^https?://')" role="ERROR"> [E0018] The value of
            ...
            <assert test="
                    if (starts-with(@target, '#') and not(starts-with(@target, '#f'))) then
                        ...
                        1" role="error"> [E0019] Value of attribute @target must have a
            ...
            <assert test="
                    if (. = tei:licence) then
                        ...
                        1" role="error"> [E0021] Value of attribute @target must have a
                corresponding @facs-value within a pb-element in the same document. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="tironianSignEtInText">
        <rule context="text()[contains(., '⁊')]">
            <report test="ancestor::tei:text" role="WARNING"> [W0005] The Unicode character 'U+204A'
    ...
    <pattern id="underbar">
        <rule context="text()[contains(., '_ _')]">
            <report test="ancestor::tei:text" role="WARNING"> [W0006] The string "_ _" has been
    ...
    <pattern id="values">
        <rule context="tei:*">
            <report test="@* = ''" role="WARNING"> [W0007] Attribute values may not be the empty
                string. </report>
        </rule>
    </pattern>
    ...
    <pattern id="editionStmt">
        <rule context="tei:editionStmt">
            <assert test="tei:p" role="ERROR"> [E0027] The information about the edition should be
    ...
    <pattern id="profileDesc">
        <rule context="tei:profileDesc">
            <assert test="tei:langUsage/tei:language" role="ERROR"> [E0035] An edition must contain
                information about the language of the text in &lt;language&gt; inside
                &lt;langUsage&gt;.</assert>
            <!--<report test="tei:creation/tei:persName[not(contains(., '(.+),(.+)'))]" role="ERROR"
